...
# 1) Fix loop headers where $i is an int counter (declared as $i=0)
$raw = [regex]::Replace($raw, '(?m)for\(\$i\s*=\s*0\s*;\s*\$i\.Value\s*-(lt|le|gt|ge|eq|ne)\b', 'for($i=0; $i -$1')

# 2) Fix common counter tests (safe for int counters, avoids PropertyNotFoundStrict)
$raw = [regex]::Replace($raw, '(?m)(?<!\.)\$i\.Value\s*-(gt|ge|lt|le|eq|ne)\b', '$i -$1')

if($raw -eq $before){ Write-Host 'NOTE: no int-counter .Value patterns found (no-op) — continuing' -ForegroundColor Yellow }
else { Write-Host 'PATCH_APPLIED: removed .Value from int loop-counter patterns' -ForegroundColor Green }
