...
# Patch ONLY inside function _ParseString(...) block, and only the "Append($ch); ... continue" line.
$start = -1; $end = -1
...
if(-not $did){ Die 'REPLACE_NOOP: no Append($ch)+continue line found inside _ParseString block' }
